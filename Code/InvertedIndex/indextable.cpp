...
void filter(std::multimap<std::string, unsigned> * tmpmap, std::string key, std::multimap<std::string, tableEntry> * table){
  ...
  for(auto itr = range.first; itr != range.second; itr++){
      keyMap.insert(std::pair<std::string, unsigned>(itr->second.path, itr->second.pos));
      //std::cout << itr->second.path << " " << itr->second.pos << std::endl;
    }
  std::multimap<std::string, unsigned> * res = new std::multimap<std::string, unsigned>;
...
std::vector<std::string>* indexTable::get(std::string key) {
  ...
  std::vector<std::string>* keys = util::strToWords(key);
    ...
    std::cout << "phrase" << std::endl;
        std::future<std::multimap<std::string, unsigned> *> * futures = new std::future<std::multimap<std::string, unsigned> *>[this->table.size()];

        for(unsigned i = 0; i < threads; i++){
            futures[i] = std::async(std::launch::async, [](std::multimap<std::string, tableEntry> * tbl, std::vector<std::string> * keys){
                std::multimap<std::string, unsigned> * tmpMap = new std::multimap<std::string, unsigned>;
                auto range = tbl->equal_range(keys->at(0));
                for (auto it = range.first; it != range.second; it++){
              tmpMap->insert(std::pair<std::string, unsigned>(it->second.path, it->second.pos));
            }
  ...
  delete keys;
    ...
    std::future<std::map<std::string, unsigned> *> * futures = new std::future<std::map<std::string, unsigned> *>[this->table.size()];
    for(unsigned i =0; i < threads; i++){
        futures[i] = std::async(std::launch::async, [](std::string key, std::multimap<std::string, tableEntry> * tbl){
            std::map<std::string, unsigned> * scoreTable = new std::map<std::string, unsigned>;
            auto range = tbl->equal_range(key);
            for(auto itr = range.first; itr != range.second; itr++){
          auto element = scoreTable->find(itr->second.path);
          if (element == scoreTable->end()){
              scoreTable->insert(std::pair<std::string, unsigned>(itr->second.path, 1));
            }
          else{
              element->second++;
            }
  ...
  for(unsigned i = 0; i < threads; i++){
      ...
      for(auto itr = tmpScoreTable->begin();itr!= tmpScoreTable->end(); itr++){
          pairs.push_back(*itr);
